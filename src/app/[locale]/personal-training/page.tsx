...
export default async function PersonalTrainingPage({
  ...
  const doc = await loadMdxFile({ locale, slug: 'personal-training' });
  return (
    <div className="mx-auto max-w-3xl px-6 sm:px-8 py-24">
      <h1 className="text-3xl font-bold tracking-tight leading-tight">{String(doc.frontmatter.title ?? 'Personal Training')}</h1>

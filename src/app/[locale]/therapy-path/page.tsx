...
export default async function TherapyPathPage({
  params,
}: {
  ...
  const doc = await loadMdxFile({ locale: 'en', slug: 'therapy-path' });

  return (
    <div className="mx-auto max-w-3xl px-6 sm:px-8 py-24">
      <h1 className="text-3xl font-bold tracking-tight leading-tight">{String(doc.frontmatter.title ?? 'Therapy path')}</h1>
